...
export function FilterBar({
  ...
  return (
    <form aria-label="Filter products" onSubmit={handleSubmit} className="text-ink">
      <details
        className="rounded-md border border-border md:border-0"
        open={open}
        onToggle={(event) => setOpen(event.currentTarget.open)}
      >
        <summary className="min-h-[44px] cursor-pointer px-3 py-2 md:hidden">Filters</summary>
        <div className="grid grid-cols-1 gap-4 p-3 md:grid-cols-2 md:p-0 lg:grid-cols-3">
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="filter-brand" className="text-sm font-medium text-ink">
              Brand
            </label>
            <select
              id="filter-brand"
