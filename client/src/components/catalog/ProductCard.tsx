...
export function ProductCard({ product }: { product: ProductCardDto }) {
  return (
    <Link
      to={`/p/${product.slug}`}
      className="group block text-ink duration-base ease-standard"
    >
      <div className="relative mb-3 aspect-square w-full overflow-hidden rounded-lg bg-surface">
        ...
        {!product.inStock ? (
          ...
          <span className="absolute left-2 top-2 rounded-sm bg-sale px-2 py-1 text-xs font-medium text-white">Sale</span>
        ) : null}
      </div>
